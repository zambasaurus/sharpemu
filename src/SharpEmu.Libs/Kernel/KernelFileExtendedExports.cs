...
public static partial class KernelMemoryCompatExports
{
    ...
    {
        // Not called out by name in #491, but it shares KernelFsyncCore with
        // fsync above and is the same bare-POSIX-name/libc-ABI situation, so
        // the same sentinel-leak gap applies here too.
        var result = KernelFsyncCore(ctx);
        return result == (int)OrbisGen2Result.ORBIS_GEN2_OK
            ? 0
            : PosixFailure(ctx, result, notFoundErrno: Ebadf);
